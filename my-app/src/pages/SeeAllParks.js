    ...
    fetch("/api/dogpark/id/" + props.id)
      ...
      .then((data) =>
        setData({
          ...
          name: data.Park_name,
          size: data.Size,
          streetnum: data.Street_number,
          street: data.Street,
          quad: data.Quadrant,
          hood: data.Neighbourhood_name,
          owner: data.Owner_ID,
        })
      );
  }, []);
  if (
    (props.allowSE && data.quad == "SE") ||
    (props.allowSW && data.quad == "SW") ||
    (props.allowNE && data.quad == "NE") ||
    (props.allowNW && data.quad == "NW") ||
    ...
    return (
      <div className="Menu-bar">
        <h4
          ...
          onClick={() => {
            window.open(
              "http://localhost:3000/ViewPark?id=" +
                props.id +
                "&name=" +
                data.name +
                "&size=" +
